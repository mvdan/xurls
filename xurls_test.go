...
var constantTestCases = []testCase{
	...
	{`matrix:r/somewhere:example.org`, true},
	{`matrix:roomid/somewhere:example.org/e/event?via=elsewhere.ca`, true},
	{`http://foo.com`, true},
	{`http://foo.co.uk`, true},
...
func TestRegexes(t *testing.T) {
	...
	doTest(t, "Public", Public(), []testCase{
		{``, nil},
		{` `, nil},
		{`:`, nil},
		{`::`, nil},
		...
		{`sms:123`, nil},
		{`xmpp:foo@bar`, nil},
		{`bitcoin:Addr23?amount=1&message=foo`, nil},
		{`cid:foo-32x32.v2_fe0f1423.png`, nil},
		{`mid:960830.1639@XIson.com`, nil},
		{`http://foo.com`, true},
		{`http://foo.co.uk`, true},
		{`http://foo.random`, nil},
		{` http://foo.com/bar `, `http://foo.com/bar`},
		...
		{`http://foo.com/path`, true},
		{`http://foo.com:8080/path`, true},
		{`http://1.1.1.1/path`, nil},
		{`http://1.1.1.1:8080/path`, nil},
		{`http://[1080::8:800:200c:417a]/path`, nil},
		{`http://[1080::8:800:200c:417a]:8080/path`, nil},

		// scheme://IPv6_addr is not valid per RFC 3987, but is supported anyway (for now).
		{`http://1080::8:800:200c:417a/path`, nil},
		{`http://2001.db8:0/path`, nil},

		{`http://中国.中国/中国`, true},
		{`http://中国.中国/foo中国`, true},
		{`http://उदाहरण.परीकषा`, nil},
		{`http://xn-foo.xn--p1acf/path`, true},
		{`what is http://foo.com?`, `http://foo.com`},
		{`go visit http://foo.com/path.`, `http://foo.com/path`},
		{`go visit http://foo.com/path...`, `http://foo.com/path`},
		...
		{`the http://foo.com!`, `http://foo.com`},
		{`https://test.foo.bar/path?a=b`, `https://test.foo.bar/path?a=b`},
		{`ftp://user@foo.bar`, nil},
		{`http://foo.com/base64-bCBwbGVhcw==`, true},
		{`http://foo.com/–`, true},
		{`http://foo.com/🐼`, true},
		{`https://shmibbles.me/tmp/自殺でも？.png`, true},
		{`randomtexthttp://foo.bar/etc`, "http://foo.bar/etc"},
		{`postgres://user:pass@host.com:5432/path?k=v#f`, nil},
		{`postgres://user:pass@host.com:5432/path?k=v#f`, nil},
		{`zoommtg://zoom.us/join?confno=1234&pwd=xxx`, nil},
		{`zoomus://zoom.us/join?confno=1234&pwd=xxx`, nil},
	})
}
